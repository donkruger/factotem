...
**First, check for existing mounts.** Groups may already have `additionalMounts` configured in the database from a previous setup. Query them:
...
for (const [jid, g] of Object.entries(groups)) {
  if (g.containerConfig?.additionalMounts?.length) {
    console.log(JSON.stringify({ group: g.name, folder: g.folder, mounts: g.containerConfig.additionalMounts }, null, 2));
  }
}
"
```

**If existing mounts are found**, check whether the mount allowlist at `~/.config/nanoclaw/mount-allowlist.json` already grants them. If the allowlist is missing or empty, you MUST pre-populate it with the paths from the existing group configs — otherwise the service will silently reject those mounts and the agent will lose access to previously configured directories (e.g. the Brain).

For each existing mount, add its `hostPath` to `allowedRoots` with `allowReadWrite` matching the mount's `readonly` field (inverted: `readonly: false` → `allowReadWrite: true`). Then AskUserQuestion to confirm: "I found existing mounts configured for these groups: {list}. I've restored them in the allowlist. Do you want to add or change any?"

**If no existing mounts are found**, proceed with the fresh setup flow:
...
**Note — REGISTERED_GROUPS=unknown (a `DB_ERROR` field is also present):** the groups DB could **not be read**; this is NOT zero groups and does not by itself set STATUS=failed. It is almost always a better-sqlite3 ABI mismatch — the Node running `npx tsx` differs from the Node that built `node_modules` / runs the service. Follow `DB_ERROR_HINT`: re-run verify with the service's Node (prepend its dir to PATH) or `npm rebuild better-sqlite3` with that Node. Do **not** "fix" this by re-registering groups.
